///
//  Generated code. Do not modify.
//  source: proto/echo.proto
//
// @dart = 2.3
// ignore_for_file: annotate_overrides,camel_case_types,unnecessary_const,non_constant_identifier_names,library_prefixes,unused_import,unused_shown_name,return_of_invalid_type,unnecessary_this,prefer_final_fields

import 'dart:async' as $async;

import 'dart:core' as $core;

import 'package:grpc/service_api.dart' as $grpc;
import 'echo.pb.dart' as $0;
export 'echo.pb.dart';

class EchoClient extends $grpc.Client {
  static final _$do_echo = $grpc.ClientMethod<$0.EchoRequest, $0.EchoReply>(
      '/echopackage.Echo/do_echo',
      ($0.EchoRequest value) => value.writeToBuffer(),
      ($core.List<$core.int> value) => $0.EchoReply.fromBuffer(value));

  EchoClient($grpc.ClientChannel channel,
      {$grpc.CallOptions options,
      $core.Iterable<$grpc.ClientInterceptor> interceptors})
      : super(channel, options: options, interceptors: interceptors);

  $grpc.ResponseFuture<$0.EchoReply> do_echo($0.EchoRequest request,
      {$grpc.CallOptions options}) {
    return $createUnaryCall(_$do_echo, request, options: options);
  }
}

abstract class EchoServiceBase extends $grpc.Service {
  $core.String get $name => 'echopackage.Echo';

  EchoServiceBase() {
    $addMethod($grpc.ServiceMethod<$0.EchoRequest, $0.EchoReply>(
        'do_echo',
        do_echo_Pre,
        false,
        false,
        ($core.List<$core.int> value) => $0.EchoRequest.fromBuffer(value),
        ($0.EchoReply value) => value.writeToBuffer()));
  }

  $async.Future<$0.EchoReply> do_echo_Pre(
      $grpc.ServiceCall call, $async.Future<$0.EchoRequest> request) async {
    return do_echo(call, await request);
  }

  $async.Future<$0.EchoReply> do_echo(
      $grpc.ServiceCall call, $0.EchoRequest request);
}
